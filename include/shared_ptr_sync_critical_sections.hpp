...
std::mutex mtx;
// Why this can not be a class member variable of shared_ptr?
// In TestD_threads 30 lines we declare sp and after we use the same one sp for thread test.
// Don't those sp have the same member std::mutex mtx?
...
public:
  ...
  {
    shared_owners = new SharedOwners{0};
  }
  ...
  shared_ptr(const shared_ptr &other)
  {
    std::lock_guard<std::mutex> lock(mtx);
    ptr = other.ptr;
    shared_owners = other.shared_owners;
    ++(*shared_owners);
  }
